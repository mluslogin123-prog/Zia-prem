...
def pe_sep() -> str:
    """Decorative separator line with premium emojis, no other emoji."""
    return f"{pe(1)}━━━━━━━━━━━━━━━━━━━━{pe(1)}"
...
# ════════════════════════════════════════════
BTN_CHECK     = "🚀 START CHECKING"
BTN_ADMIN     = "⚡ ADMIN PANEL"
BTN_STOP      = "🛑 STOP SESSION"
BTN_STATUS    = "ℹ️ MY STATUS"
BTN_RESULTS   = "📥 GET RESULTS"
BTN_HITS_ON   = "🔔 NOTIFS: ON"
BTN_HITS_OFF  = "🔕 NOTIFS: OFF"
BTN_DELETE    = "🗑️ DELETE FILE"

BTN_START_NOW = "🔥 RUN ENGINE NOW"
BTN_LVL_MENU  = "📊 LEVEL FILTER"
BTN_CF_MENU   = "🛡️ CLEAN FILTER"

BTN_LVL_ALL   = "🌐 ALL LEVELS"
BTN_LVL_100   = "🎖️ LEVEL 100+"
BTN_LVL_200   = "🎖️ LEVEL 200+"
BTN_LVL_300   = "🎖️ LEVEL 300+"
BTN_LVL_400   = "🎖️ LEVEL 400+"

BTN_CF_BOTH   = "💎 ALL HITS"
BTN_CF_CLEAN  = "✨ CLEAN ONLY"
BTN_CF_DIRTY  = "💀 DIRTY ONLY"

BTN_CONTINUE  = "▶️ CONTINUE"
BTN_STOP_GET  = "📥 STOP & SAVE"
...
def stats_card(done,total,stats,ll="",cl="",result_folder=None,start_ts=None):
    ...
    filled=pct//10
    # Premium progress bar with emoji blocks
    bar_filled = "💎" * filled + "▫️" * (10 - filled)
    sl=f"{pe(1)} <b>FILTER:</b> <code>{ll or 'ALL'}</code> | <code>{cl or 'BOTH'}</code>\n"
    valid=stats.get('valid',0); invalid=stats.get('invalid',0)
    has_codm=stats.get('has_codm',0); total_done=stats.get('total',done)
    hit_rate=f"{has_codm/valid*100:.1f}%" if valid>0 else "0%"
    speed_s=_fmt_speed(done,start_ts)
    eta_s=_fmt_eta(done,total,start_ts)
    speed_line=f" ⚡ <b>SPEED:</b> <code>{speed_s or '...'}</code>  ⏳ <b>ETA:</b> <code>{eta_s or '...'}</code>\n"

    # Realistic loading animation
    loading_frames = ["🕛", "🕒", "🕕", "🕘"]
    frame = loading_frames[int(time.time() * 2) % len(loading_frames)]
    
    base=(f"{pe(5)} <b>ᴛᴏꜱʜɪ ᴘʀᴏ ᴠ6.0</b> {pe(5)}\n"
          f"{pe_sep()}\n"
          f" {frame} <b>STATUS:</b> <code>CHECKING...</code>\n"
          f" 📊 <b>PROGRESS:</b> <code>{pct}%</code>\n"
          f" {bar_filled}\n"
          f" ✅ <b>DONE:</b> <code>{done:,}</code> / <code>{total:,}</code>\n"
          f"{speed_line}"
          f"{pe_sep()}\n"
          f" 💎 <b>LIVE HITS:</b> <code>{has_codm:,}</code>\n"
          f" 🛡️ <b>VALID:</b> <code>{valid:,}</code>\n"
    ...
    if result_folder:
        try:
            ...
            if live_codm>0 and hits>0 and hits!=live_codm:
                ...
                hits=live_codm
            if hits>0:
                lvl_lines=f"{pe(3)} <b>Level Distribution</b> {pe(3)}\n"
                for rng,cnt in lvl.items():
                    pct2=cnt/hits*100
                    bar2="█"*int(pct2//10)+"░"*(10-int(pct2//10))
                    lvl_lines+=f"  {rng:<8}: [{bar2}] {cnt} ({pct2:.1f}%)\n"
                ctr_lines=f"{pe(3)} <b>Server Distribution</b> {pe(3)}\n"
    ...
    return base+extra+sl+f"{pe(1)} /stop — stop  |  /cancel — cancel"
...
async def deliver_results(bot,chat_id,uid,zip_paths,stats,combo_file=None,note="",partial=False):
    ...
    try:
        _body = (f"{pe(5)} <b>{label.upper()}</b> {pe(5)}\n"
                  f"{pe_sep()}\n"
                  f" ✅ <b>PROCESSED:</b> <code>{t:,}</code>\n"
                  f" 💎 <b>TOTAL HITS:</b> <code>{stats.get('has_codm',0):,}</code>\n"
                  f"{pe_sep()}\n"
                  f" 🛡️ <b>VALID:</b> <code>{stats.get('valid',0):,}</code>\n"
                  f" 💀 <b>INVALID:</b> <code>{stats.get('invalid',0):,}</code>\n"
                  f" ✨ <b>CLEAN:</b> <code>{stats.get('clean',0):,}</code>\n"
                  f" 🌑 <b>DIRTY:</b> <code>{stats.get('not_clean',0):,}</code>\n"
                  f"{pe_sep()}\n"
                  f" <i>{'System still checking...' if partial else 'All results delivered!'}</i>")
...
async def cmd_start(update,context):
    ...
    bot_name = cfg.get("bot_name","ᴛᴏꜱʜɪ ᴘʀᴏ ᴠ6.0")
    ...
    if not ud.get("activated") and not is_admin(tg.id,cfg):
        ...
        base_txt = (
            ...
            f"{pe_sep()}\n"
            f"{pe(2)} Welcome, <b>{tg.first_name}</b>! {pe(2)}\n"
            f"{pe_sep()}\n"
            f"{pe(3)} Redeem a key to get started! {pe(3)}\n\n"
            f"Usage: <code>/redeem YOUR_KEY</code>\n"
            f"{pe_sep()}\n"
            f"{pe(1)} Contact admin to purchase a key."
        )
        if wc: base_txt += f"\n{pe_sep()}\n{pe(1)} {wc}"
        await update.message.reply_text(base_txt, parse_mode=ParseMode.HTML, reply_markup=ReplyKeyboardRemove()); return
    ...
    m = await update.message.reply_text(
        f"{pe(5)} <b>ᴛᴏꜱʜɪ ᴘʀᴏ ᴠ6.0</b> {pe(5)}\n"
        f"{pe_sep()}\n"
        f" 👤 <b>USER:</b> <code>{tg.first_name}</code> {badge}\n"
        f" 🆔 <b>ID:</b> <code>{tg.id}</code>\n"
        f" 📊 <b>CHECKED:</b> <code>{ud.get('total_checked',0):,}</code>\n"
        f" 🚀 <b>SESSIONS:</b> <code>{ud.get('sessions_count',0)}</code>\n"
...
async def cmd_redeem(update,context):
    ...
    await update.message.reply_text(
        ...
        f" 📅 <b>EXPIRY:</b> <code>{fmt_expiry(ud['key_expires_at'])}</code>\n"
        f"{pe_sep()}\n"
        f" 🚀 <b>Tap /start to start checking!</b>",
        parse_mode=ParseMode.HTML)

async def cmd_help(update, context):
    help_text = (
        f"{pe(5)} <b>ɪ ʜᴇʟᴩ ᴄᴇɴᴛᴇʀ</b> {pe(5)}\n"
        f"{pe_sep()}\n"
        f" 🛠️ <b>COMMANDS:</b>\n"
        f" ┣ /start - 🏠 Main Menu\n"
        ...
        f" ┣ /check - 📊 Live Stats\n"
        f" ┣ /stop - 🛑 Stop Checker\n"
        f" ┣ /profile - 👤 My Account\n"
        f" ┗ /about - ℹ️ System Info\n"
        f"{pe_sep()}\n"
        f" 📖 <b>HOW TO USE:</b>\n"
        f" 1️⃣ Type /start to open menu\n"
        f" 2️⃣ Upload your <code>.txt</code> combo\n"
...
async def cmd_about(update, context):
    about_text = (
        f"{pe(5)} <b> ᴘʀᴏ ᴠ6.0</b> {pe(5)}\n"
        f"{pe_sep()}\n"
        f" 🚀 <b>ᴇɴɢɪɴᴇ:</b> <code>ᴛᴏꜱʜɪ-x64 ʜʏᴩᴇʀ</code>\n"
...
async def cmd_profile(update, context):
    ...
    ud, _ = get_or_create_user(uid)
    profile_text = (
        f"{pe(5)} <b>USER ACCOUNT</b> {pe(5)}\n"
        f"{pe_sep()}\n"
        f" 👤 <b>NAME:</b> <code>{tg.first_name}</code>\n"
        f" 🆔 <b>ID:</b> <code>{tg.id}</code>\n"
        f" 🎖️ <b>RANK:</b> <code>{'⭐ VIP' if ud.get('vip') else '👤 USER'}</code>\n"
        f" 📅 <b>JOINED:</b> <code>{ud.get('joined', 'N/A')[:10]}</code>\n"
        f"{pe_sep()}\n"
        f" 📊 <b>LIFETIME STATS:</b>\n"
        f" ┣ 🏁 Total Checked: <code>{ud.get('total_checked', 0):,}</code>\n"
        f" ┣ 💎 Total Hits: <code>{ud.get('total_hits', 0):,}</code>\n"
        f" ┗ 🚀 Sessions: <code>{ud.get('sessions_count', 0)}</code>\n"
        f"{pe_sep()}\n"
        f" 🔑 <b>SUBSCRIPTION:</b>\n"
        f" ┗ ⌛ Expiry: <code>{fmt_expiry(ud.get('key_expires_at'))}</code>\n"
        f"{pe_sep()}\n"
    )
...
async def on_callback(update,context):
    ...
    if data=="start_check":
        ...
        await query.edit_message_text(
        f"{pe(5)} <b>ENGINE SETTINGS</b> {pe(5)}\n"
        f"{pe_sep()}\n"
        f" 📁 <b>FILE:</b> <code>{fn}</code>\n"
        f" 🎖️ <b>LEVEL:</b> <code>{ll}</code>\n"
        f" ✨ <b>FILTER:</b> <code>{cl}</code>\n"
        f"{pe_sep()}\n"
        f" <i>Configure your engine parameters:</i>",
        reply_markup=kb_settings(uid), parse_mode=ParseMode.HTML)
        ...
        await query.edit_message_text(
            f"{pe(3)} <b>ᴛᴏꜱʜɪ ᴇɴɢɪɴᴇ ɪɴɪᴛɪᴀʟɪᴢɪɴɢ</b> {pe(3)}\n"
            f"{pe_sep()}\n"
            f" ⏳ <code>[▒▒▒▒▒▒▒▒▒▒] 0%</code>\n"
            f" ⚙️ Loading modules...\n"
            f" 🛡️ Anti-ban bypass active...\n"
            f" 🚀 Optimized for speed...\n"
            f"{pe_sep()}\n"
            f"<i>System will start in a moment...</i>",
            parse_mode=ParseMode.HTML)
...
async def _show_admin_panel(msg, cfg):
    ...
    uptime_s5=int(time.time()-_railway_start); h5,m5=uptime_s5//3600,(uptime_s5%3600)//60
    bot_name5=cfg.get("bot_name","ᴛᴏꜱʜɪ ᴄʜᴇᴄᴋᴇʀ ʙᴏᴛ")
...
def _admin_status_text(cfg, users):
    ...
    return (f"{pe(5)} <b>ᴛᴏꜱʜɪ ᴀᴅᴍɪɴ ᴅᴀꜱʜʙᴏᴀʀᴅ</b> {pe(5)}\n"
            f"{pe_sep()}\n"
            f" 👥 <b>TOTAL USERS:</b> <code>{len(users)}</code>\n"
            f" ┣ ✅ Activated: <code>{ac}</code>\n"
            f" ┣ 🚫 Banned: <code>{bc}</code>\n"
            f" ┗ ⭐ VIP: <code>{vc}</code>\n"
            f"{pe_sep()}\n"
            f" ⚡ <b>ENGINE STATUS:</b>\n"
            f" ┣ 🔄 Active: <code>{live}/{MAX_CONCURRENT_CHECKERS}</code>\n"
            f" ┣ 🔒 Access: <code>{lock_s}</code>\n"
            f" ┗ 🚀 Uptime: <code>{h}h {m}m</code>\n"
            f"{pe_sep()}\n"
            f" ⚙️ <b>LIMITS:</b>\n"
            f" ┣ 👤 User: <code>{cfg.get('global_limit') or '∞'}</code>\n"
            f" ┗ ⭐ VIP: <code>{cfg.get('vip_limit') or '∞'}</code>\n"
            f"{pe_sep()}\n"
            f" <i>Select a management module:</i>")
...
async def cmd_set_commands(update, context):
    ...
    reseller_cmds2 = [
        ...
        BotCommand("about",          "ℹ ᴀʙᴏᴜᴛ ᴛᴏꜱʜɪ ʙᴏᴛ"),
    ]
...
def main():
    cfg=load_config()
    if not cfg.get("bot_token") or cfg["bot_token"]=="YOUR_BOT_TOKEN_HERE":
    ...
    print(f"  ʙᴏᴛ ꜱᴛᴀʀᴛɪɴɢ — @{cfg['channel_username']}  |  Slots: {MAX_CONCURRENT_CHECKERS}")
...
if __name__ == "__main__":
    ...
    app.add_handler(CommandHandler("check", cmd_check_progress))
    app.add_handler(CommandHandler("stop", _do_stop))
    app.add_handler(CommandHandler("status", cmd_status))
    app.add_handler(CommandHandler("myresultsfile", cmd_get_results))
    app.add_handler(CommandHandler("deletefile", cmd_delete_file))
    app.add_handler(CommandHandler("clean", cmd_clean_combo))
    app.add_handler(CommandHandler("cancel", cmd_cancel_session))
    app.add_handler(CommandHandler("hitson", cmd_hits_on))
    app.add_handler(CommandHandler("hitsoff", cmd_hits_off))
    ...
    print(f"  Handlers registered. Running...")
